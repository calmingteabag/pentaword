class PentaGame {
    ...
    keyPressAlpha(usrkey) {
        let usrKey = new PentaGame()
        // Populates display if user presses keys on physical keyboard

        if (this.item_pos < 5 && this.isAlpha(usrkey.key) == true && usrkey.key != '' && localStorage.getItem('game_state') == 'active') {
            let rowlist = document.getElementsByClassName('row_try')[this.active_row];
            let curr_char = rowlist.children;
    ...
    helpToggle(helpElement) {
        ...
        if (helpToggle.style.visibility == 'hidden') {
            this.helpShow('help_wrapper').bind(this)
        } else {
            this.helpClose('help_wrapper').bind(this)
        }
    };
    ...
    checkWord() {
        ...
        let check_pos = 0
        let daily_word_arr = this.word.toUpperCase().split('')
    ...
    showTimerDOM() {
        ...
        let second_a = this.leadZerotime(second)
    ...
    subWord() {
        ...
        */
        ...
        let dailyWordArr = this.word.toUpperCase().split('')
        ...
        if (this.active_row < 5 && this.item_pos == 5 && this.compareArr(this.arr, dailyWordArr) == false && localStorage.getItem('game_state') == 'active') {
            this.checkWord()
            this.tried_words.push(this.arr)
            this.active_row++
            this.item_pos = 0
            this.arr = []
    ...
    addCharListener(classname, atrib) {
        ...
        for (let elem of btn_elem) {
            ...
            elem.addEventListener("click", () => { this.getKeyPress(current_attb) }, false)
        }
    };
    ...
    exitStatListener(exitStat) {
        ...
        exit_btn.addEventListener("click", () => { this.closeStat('score_wrapper') }, false)
    };

    showStatsListener(statElement) {
        let stat_listener = document.getElementById(statElement)
        stat_listener.addEventListener("click", () => { this.statToggle('score_wrapper') }, false)
    };

    helpCloseStatListener(helpElement) {
        let exit_btn = document.getElementById(helpElement)
        exit_btn.addEventListener("click", () => { this.helpClose('help_wrapper') }, false)
    };

    helpShowStatsListener(helpStat) {
        let stat_listener = document.getElementById(helpStat)
        stat_listener.addEventListener("click", () => { this.helpToggle('help_wrapper') }, false)
    };

    addGambiarraListener(gambiarra) {
        let gambi = document.getElementById(gambiarra)
        gambi.addEventListener("click", () => { this.resetGambiarra() }, false)
    };

    // Loads all listeners
    loadAllListeners() {
        // When using addeventlistener with '() => {this.method()}', the 'this' will reference the document. So everything inside
        // this.method() that uses document for something (document.getElementById for example) will work. IF you try
        // to use the regular 'function () {this.method()}', the 'this' will reference current object. Since there is no 'document'
        // and therefore no DOM Elements, it will return 'this.method is not a function' error

        // One way to "fix" this is 
        let localInstance = new PentaGame()
        // document.addEventListener("DOMContentLoaded", this.rowGlowAnimate, false);
        // document.addEventListener("DOMContentLoaded", () => { this.rowGlowAnimate() }, false);
        document.addEventListener("DOMContentLoaded", function () { localInstance.rowGlowAnimate().bind(this) }, false);
        ...
        document.addEventListener("DOMContentLoaded", function () { localInstance.helpShowStatsListener('show_help') }, false);
        document.addEventListener("keyup", function () { localInstance.keyPressAlpha() }, false);
